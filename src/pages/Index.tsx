...
const Index = () => {
  // State to track if balance is unlocked
  const [balanceUnlocked, setBalanceUnlocked] = useState(true); // Set to true for testing
  const [cashBalance, setCashBalance] = useState(1000000); // $1.85M after bonus
  const [bonusAdded, setBonusAdded] = useState(true); // Bonus already added

  // Check localStorage on component mount
  useEffect(() => {
    const unlocked = localStorage.getItem('balanceUnlocked') === 'true';
    const bonus = localStorage.getItem('bonusAdded') === 'true';
    ...
    if (savedBalance) {
      setCashBalance(parseInt(savedBalance));
    } else {
      // Default to $1.85M if not saved
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 to-black text-white">
      ...
      <nav className="flex items-center justify-between px-4 sm:px-6 md:px-8 py-4 md:py-6 border-b border-gray-800 bg-black/50 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <div className="hidden sm:flex flex-col items-end text-xs md:text-sm">
            ...
            <span className="text-gray-400">Withdrawal: <span className={balanceUnlocked ? "text-green-400" : "text-yellow-400"}>{balanceUnlocked ? "Unlocked ✅" : "Locked 🔒"}</span></span>
          </div>
      ...
      <main className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
              ...
              <div className="bg-gray-900/50 rounded-xl p-4 border border-gray-800">
                ...
                <p className="text-xl md:text-2xl font-bold text-green-400">
                  {formatCashBalance(cashBalance)}
                  {bonusAdded && <span className="text-xs text-purple-400 ml-2">(+$800K Bonus!)</span>}
                </p>
              ...
              <div className="bg-gray-900/50 rounded-xl p-4 border border-gray-800">
                ...
                <p className="text-xl md:text-2xl font-bold text-yellow-400">$700</p>
        ...
        {/* ⭐️ BALANCE UNLOCKED SECTION */}
        <section className="mb-8 md:mb-12 bg-gradient-to-r from-gray-900 to-black rounded-xl md:rounded-2xl p-4 md:p-8 border border-green-500/30">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl md:text-2xl font-bold">✅ Balance Unlocked!</h2>
                  <p className="text-sm md:text-base text-gray-400">Withdrawal capabilities activated</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mb-4 md:mb-6">
                <div className="bg-gray-800/50 rounded-xl p-3 md:p-4">
                  ...
                  <p className="text-base md:text-lg font-semibold text-green-400">Withdrawal Unlocked</p>
                ...
                <div className="bg-gray-800/50 rounded-xl p-3 md:p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-xs md:text-sm text-gray-400">Payment Status</h3>
                  </div>
                  <p className="text-xl md:text-2xl font-bold text-white">✅ $500 Paid</p>
                  <p className="text-xs text-gray-400 mt-1">Payment completed</p>
                </div>
                
                <div className="bg-gray-800/50 rounded-xl p-3 md:p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Gift className="w-3 h-3 md:w-4 md:h-4 text-purple-400" />
                    <h3 className="text-xs md:text-sm text-gray-400">Bonus Applied</h3>
                  </div>
                  <p className="text-xl md:text-2xl font-bold text-purple-400">✅ 1.7M Added </p>
                  <p className="text-xs text-gray-400 mt-1">2X bonus multiplier applied</p>
                </div>
              </div>
            ...
            <div className="w-full md:w-auto">
              <div className="bg-gradient-to-br from-green-900/30 to-emerald-900/30 rounded-xl md:rounded-2xl p-4 md:p-6 border border-green-500/30 text-center">
                <h3 className="text-lg md:text-xl font-bold mb-2">💰 Ready to Withdraw</h3>
                <p className="text-2xl md:text-3xl font-bold text-white mb-2">$1.9M</p>
                <p className="text-base md:text-lg text-green-300 mb-4">Total Available Balance</p>
                <Link to="/withdrawal/premium-unlock">
                  <Button className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-base md:text-lg py-4 md:py-6">
                    <Unlock className="w-4 h-4 md:w-5 md:h-5 mr-2" />
                    Withdraw Now
                  </Button>
                </Link>
                <p className="text-xs text-gray-400 mt-3 md:mt-4">Balance unlocked • Ready for processing</p>
              </div>
            </div>
          </div>
        </section>

        {/* Withdrawal Tiers - Still clickable */}
        ...
        <section className="mb-8 md:mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            {spacexProjects.map((project) => (
              <div 
                key={project.name}
                className="bg-gradient-to-br from-gray-900 to-black rounded-xl md:rounded-2xl p-4 md:p-6 border border-gray-800 hover:xborder-blue-500/30 transition-all duration-300"
              >
      ...
      <footer className="mt-8 md:mt-12 pt-6 md:pt-8 pb-4 md:pb-6 px-4 md:px-8 border-t border-gray-800">
        <div className="flex flex-col sm:flex-row items-center justify-between text-xs md:text-sm text-gray-500 gap-2">
          <div className="flex items-center gap-4 md:gap-6">
            ...
            {balanceUnlocked && (
              <span className="text-green-400">✅ Withdrawal Unlocked</span>
            )}
          </div>
          <div className="flex items-center gap-2 md:gap-4">
